...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑教材入库</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>教材入库管理</span></li>
								<li class="breadcrumb-item active"><span>编辑教材入库</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">教材入库信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>入库编号</label>
										<input id="rukubianhao" name="rukubianhao" class="form-control" value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>教材名称</label>
									...
									<div class="form-group col-md-6">
										...
										<input name="chubanshe" id="chubanshe" name="chubanshe" class="form-control" placeholder="出版社">
											...
											<div class="form-group  col-md-12" >
												<label>入库详情</label>
												<div><div class="upload"><input id="rukuxiangqingupload" name="file" type="file">选择文件</div>
												<script id="rukuxiangqingEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var rukuxiangqingUe = UE.getEditor('rukuxiangqingEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="rukuxiangqing" id="rukuxiangqing-input">
												</div>
											</div>	
	...
	<script>
		...
		function rukubianhaoUuid(){
			if($("#rukubianhao").val() == null || $("#rukubianhao").val() == "null" || $("#rukubianhao").val() == ""){
				$("#rukubianhao").attr("value",Math.round(new Date().getTime()/1000));
			}
		}				
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "jiaocairuku/info/" + id,
		...
		function upload(){
			...
			$('#rukuxiangqingupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('rukuxiangqingEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("jiaocairuku/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("jiaocairuku/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.rukuxiangqing != null && ruleForm.rukuxiangqing != 'null' && ruleForm.rukuxiangqing != ''){
				var rukuxiangqingMes = '' + ruleForm.rukuxiangqing;
				var rukuxiangqingUeditor = UE.getEditor('rukuxiangqingEditor');
				rukuxiangqingUeditor.ready(function() {
					rukuxiangqingUeditor.setContent(rukuxiangqingMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('rukuxiangqingEditor').hasContents()){
				$('#rukuxiangqing-input').attr('value',UE.getEditor('rukuxiangqingEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("jiaocairuku/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			$("#rukubianhao").val(ruleForm.rukubianhao);	
			...
			$("#rukuxiangqing").val(ruleForm.rukuxiangqing);	
		}		
		//图片显示
		function showImg(){
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.rukubianhao != null && res.data.rukubianhao != ''){

						$("#rukubianhao").val(res.data.rukubianhao);
						$("#rukubianhao").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.rukuxiangqing != null && res.data.rukuxiangqing != ''){

						$("#rukuxiangqing").val(res.data.rukuxiangqing);
                        var rukuxiangqingMes = '' + res.data.rukuxiangqing;
                        var rukuxiangqingUeditor = UE.getEditor('rukuxiangqingEditor');
                        rukuxiangqingUeditor.ready(function() {
                            rukuxiangqingUeditor.setContent(rukuxiangqingMes);
                        });
